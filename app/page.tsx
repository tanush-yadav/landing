...
type InsightTone = 'critical' | 'warning' | 'info' | 'positive';
type SentenceType = 'missing' | 'losingGround' | 'quickWin' | 'newTrend';
...
const INSIGHT_CARDS: InsightCard[] = [
  {
    query: 'best SOC2 automation for startups',
    intent: 'Best',
    status: 'Missing',
    tone: 'critical',
    competitor: 'Vanta',
    whyMatters: '1,300 daily queries',
    recommendedAction:
      'Write a website page targeting this query with Best intent to outperform Vanta on this query.',
    include: [
      'H2: Problem for startup security leads evaluating SOC2 automation.',
      'Table: You vs Vanta on evidence collection, audit support, Slack alerts, shared evidence folder, and pricing control.',
      'Pricing summary with startup bundle calculator and transparent implementation fees.',
      'Setup steps: connect cloud apps, map controls, invite auditors, launch Slack alerts.',
      'Internal links to SOC2 checklist and audit readiness workspace plus FAQ on timeline, integrations, and continuous monitoring.',
    ],
    effort: 'Low',
    expectedVisits: '9 to 14',
    expectedTrials: '3',
    sentenceType: 'missing',
  },
  {
    query: 'SOC2 automation pricing',
    intent: 'Pricing',
    status: 'Dropping to #5',
    tone: 'warning',
    competitor: 'Drata',
    whyMatters: '740 daily queries',
    recommendedAction:
      'Refresh the pricing page. Add a compare table, usage tiers, and hidden costs note.',
    include: [
      'H2: Pricing problem for growth-stage teams evaluating SOC2 automation.',
      'Table: You vs Drata across subscription tiers, usage limits, implementation fees, support SLAs, and compliance guarantees.',
      'Pricing summary with calculator for seats and automated evidence add-ons.',
      'Setup steps: audit current pricing page, slot in calculator, add hidden costs explainer.',
      'Internal links to billing FAQ and compliance ROI report with FAQ on renewals and overages.',
    ],
    effort: 'Low',
    expectedVisits: '5 to 7',
    expectedTrials: '2',
    sentenceType: 'losingGround',
  },
  {
    query: 'Notion alternative for engineering notes',
    intent: '"Vs" comparison',
    status: 'Not cited',
    tone: 'info',
    competitor: 'Notion',
    whyMatters: '360 daily queries',
    recommendedAction:
      'Write a website page targeting this query with Comparison intent to outperform Notion.',
    include: [
      'H2: Problem for engineering leaders needing structured delivery notes.',
      'Table: You vs Notion on offline mode, API limits, SSO, export fidelity, and engineering templates.',
      'Pricing summary highlighting engineering team bundles with ROI calculator.',
      'Setup steps: migrate spaces, assign templates, enable SSO, sync repos.',
      'Internal links to engineering docs and workspace migration guide with FAQ on API access and permissions.',
    ],
    effort: 'Medium',
    expectedVisits: '4 to 8',
    expectedTrials: '1',
    sentenceType: 'quickWin',
    page: 'product overview page',
  },
  {
    query: 'security review checklist',
    intent: 'How-to',
    status: 'Lost to Competitor B',
    tone: 'warning',
    competitor: 'Competitor B',
    whyMatters: '280 daily queries',
    recommendedAction: 'Publish a checklist page with a downloadable PDF.',
    include: [
      'H2: Problem for security teams preparing for procurement reviews.',
      'Table: You vs Competitor B on review speed, templates, collaboration, audit support, and integrations.',
      'Pricing summary with calculator for reviewer seats and automation modules.',
      'Setup steps: configure review workflow, attach evidence, enable approver notifications.',
      'Internal links to buyer enablement guide and security hub with FAQ on review cadence and approver roles.',
    ],
    effort: 'Medium',
    expectedVisits: '3 to 6',
    expectedTrials: '1',
    sentenceType: 'losingGround',
  },
  {
    query: 'SOC2 risk register template',
    intent: 'Template',
    status: 'New opportunity',
    tone: 'positive',
    competitor: 'Secureframe',
    whyMatters: '210 daily queries',
    recommendedAction: 'Publish a template page with CSV download and example fields.',
    include: [
      'H2: Problem for compliance owners compiling SOC2 risks quickly.',
      'Table: You vs Secureframe on template depth, automation, evidence linking, export formats, and reviewer collaboration.',
      'Pricing summary with calculator for risk management add-on.',
      'Setup steps: import template, assign owners, review controls, export to auditors.',
      'Internal links to risk management guide and evidence automation article with FAQ on CSV exports and versioning.',
    ],
    effort: 'Low',
    expectedVisits: '2 to 4',
    expectedTrials: '1',
    sentenceType: 'newTrend',
    trendPercent: 28,
  },
];

const statusToneStyles: Record<InsightTone, string> = {
...
const sentenceTemplates: Record<SentenceType, (insight: InsightCard) => string> = {
  missing: (insight) =>
    `Write a website page targeting this query with ${insight.intent} intent to outperform ${insight.competitor}. You are not cited today.`,
  losingGround: (insight) =>
    `Refresh the existing page for ${insight.query} to recover rank from ${insight.competitor}. Add the items below.`,
  quickWin: (insight) =>
    `Add a short section to your ${insight.page ?? 'existing page'} to capture ${insight.query}. Keep the structure below.`,
  newTrend: (insight) =>
    `Publish a lightweight page for ${insight.query}. Interest is up ${insight.trendPercent ?? 0}% this week.`,
};
            ...
            {[
              ...
              <div
                ...
                className={`flex items-center justify-between p-2.5 rounded-md text-sm ${
                  item.highlight ? 'bg-blue-50 border border-blue-200/60' : 'bg-white'
                }`}
              >
                <span className={`font-medium ${item.highlight ? 'text-blue-900' : 'text-gray-700'}`}>
                  {item.name}
                </span>
                  ...
                  <div className="w-32 sm:w-40 bg-gray-200 rounded-full h-2 overflow-hidden">
                    <div
                      className={`h-2 rounded-full transition-all duration-1000 ${
                        item.highlight ? 'bg-blue-500' : 'bg-gray-400'
                      }`}
      ...
      <section className="max-w-[1400px] mx-auto px-8 sm:px-16 lg:px-[20%] pt-20 pb-32">
        <div className="text-center max-w-3xl mx-auto">
          ...
          <ScrollReveal delay={0.1}>
            <h1
              className="text-3xl sm:text-4xl md:text-5xl font-normal mb-4 leading-[1.15]"
              style={{ letterSpacing: '-0.02em',  fontWeight: 400 }}
            >
              <span className="text-black">Show up in ChatGPT for your buyers&apos; top questions</span>
            </h1>
            <p className="text-gray-400 text-xl sm:text-2xl md:text-3xl font-normal mb-8 leading-[1.25]" style={{ letterSpacing: '-0.02em' }}>
              Get cited in the answers that drive signups. See wins in 6 weeks.
            </p>
          </ScrollReveal>
