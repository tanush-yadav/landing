...
export function getPostBySlug(slug: string): BlogPost | null {
  ...
  const post: BlogPost = {
    ...
    excerpt: data.description || data.excerpt || content.substring(0, 200) + '...',
    content: content,
    author: {
      name: 'Sophia',
...
function extractTagsFromContent(content: string): string[] {
  ...
  const keywords = [
    'AI Automation', 'Productivity', 'Workflows', 'Integration',
    'Analytics', 'Strategy', 'Best Practices', 'Team Management'
  ];
...
export function getCategoriesWithCounts(): BlogCategory[] {
  ...
  categories.forEach(category => {
    category.postCount = posts.filter(post => 
