...
function workflowToConfig(workflow: TaskWorkflow): DemoConfig | null {
  ...
  const agent: Agent = {
    ...
    role: `AI ${workflow.category.charAt(0).toUpperCase() + workflow.category.slice(1)} Specialist`
  }
  ...
  workflow.steps.forEach(step => {
    ...
    if (step.slackMessage) {
      slackMessages.push({
        ...
        author: step.slackMessage.author === workflow.employee ? workflow.employee : step.slackMessage.author,
        avatar: step.slackMessage.author === workflow.employee ? agent.avatar : 'U',
        ...
        timestamp: new Date(step.slackMessage.timestamp || Date.now()).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }),
        delay: msgIndex * 500
      })
    }
  })
...
const InteractiveDemo = memo(({
  ...
  const startDemoSequence = useCallback(async () => {
    ...
    try {
      ...
      setDemoState(prev => {
        const newState: DemoState = { ...prev, phase: 'analyzing' as const, ticketCreated: true }
      ...
      safeSetTimeout(() => {
        setDemoState(prev => ({ ...prev, phase: 'creating-subtasks' as const }))
  ...
  console.log('Rendering InteractiveDemo - demoState:', demoState, 'currentTask:', currentTask)
          ...
          >
            ...
            <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100">
              <div className="flex items-center gap-3">
                ...
                <span className="text-gray-800 font-medium text-sm">Linear</span>
              </div>
                ...
                {demoState.ticketCreated && (
                  ...
                  >
                    ...
                    <h3 className="text-gray-900 text-base font-medium mb-4">
                      {currentTask?.title || 'Fix authentication bug in production'}
                    </h3>
                      ...
                      {demoState.phase !== 'idle' && (
                        ...
                        >
                          <div className="w-6 h-6 bg-purple-500 rounded-full flex items-center justify-center flex-shrink-0">
                            <span className="text-white text-[10px] font-semibold">{currentTask?.agent.avatar || 'Z'}</span>
                          </div>
                          <div className="flex-1">
                            <p className="text-sm text-gray-700">
                              <span className="font-medium text-gray-900">{currentTask?.agent.name || 'Zoe'}</span>
                              <span className="text-gray-600"> is analyzing {currentTask?.description || 'the requirements'}</span>
                            </p>
                      ...
                      {demoState.subtasksCreated && (
                        ...
                        >
                          ...
                          <div className="flex-1">
                            <p className="text-sm text-gray-700">
                              <span className="font-medium text-gray-900">{currentTask?.agent.name || 'Zoe'}</span>
                              <span className="text-gray-600"> created {currentTask?.subtasks.length || 5} sub-issues</span>
                            </p>
          ...
          <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden transition-all duration-200 hover:-translate-y-1 hover:shadow-xl will-change-transform">
            ...
            <div className="bg-[#4A154B] px-6 py-4 flex items-center justify-between">
              <div className="flex items-center gap-2">
                ...
                <span className="text-white font-medium text-sm">#development</span>
              </div>
              <div className="flex items-center gap-3">
